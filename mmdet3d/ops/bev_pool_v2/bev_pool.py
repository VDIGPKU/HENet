...
__all__ = ['bev_pool_v2', 'TRTBEVPoolv2', 'TRTBEVPoolv2WithZ']
...
class QuickCumsumCuda(torch.autograd.Function):
    ...
    def forward(ctx, depth, feat, ranks_depth, ranks_feat, ranks_bev,
        ...
        if torch.onnx.is_in_onnx_export(): # we export onnx by cpu
        # if False:
            out = feat.new_zeros(bev_feat_shape).cuda()
            bev_pool_v2_ext.bev_pool_v2_forward(
                ...
                ranks_feat.cuda(),
                ranks_bev.cuda(),
                interval_lengths.cuda(),
                interval_starts.cuda(),
            )
...
class TRTBEVPoolv2(torch.autograd.Function):
    ...
    def symbolic(g,
        ...
        return g.op(
            ...
            output_width_i=output_width) # i 表示 int
    ...
    def forward(g,
        ...
        bev_feat_shape = (int(depth.shape[0]), 1, output_height, output_width,
                          int(feat.shape[-1]))  # (B, Z, Y, X, C)
...
class TRTBEVPoolv2WithZ(torch.autograd.Function):
    ...
    def forward(g,
        ...
        depth = depth.unsqueeze(0)
        # change Z manually
        bev_feat_shape = (int(depth.shape[0]), output_z, output_height, output_width,
                          int(feat.shape[-1]))  # (B, Z, Y, X, C)
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               bev_feat_shape, interval_starts,
                               interval_lengths)
        # bev_feat = bev_feat.squeeze(2)
        bev_feat = bev_feat.permute(0, 2, 3, 4, 1)
        return bev_feat

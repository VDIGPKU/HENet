...
# for orin cannot use msmv sampling, change it to deform-attention
# MSMV_CUDA = False
    ...
    def forward(ctx, feat_c2, feat_c3, feat_c4, feat_c5, sampling_locations, scale_weights):
        ...
        if torch.onnx.is_in_onnx_export(): # we export onnx by cpu
            return _ms_deform_attn_cuda_c2345_forward(
                feat_c2.cuda(), feat_c3.cuda(), feat_c4.cuda(), feat_c5.cuda(),
                sampling_locations.cuda(), scale_weights.cuda()).cpu()
        else:
            return _ms_deform_attn_cuda_c2345_forward(
...
def msmv_sampling_trt(mlvl_feats, sampling_locations, scale_weights):
    return MSMVSamplingC2345.apply(*mlvl_feats, sampling_locations, scale_weights)


class TRTMSMVSampling(torch.autograd.Function):
    ...
    def symbolic(g,
        ...
        return g.op(
            # 'mmdeploy::msmv_sampling',
            'custom::MsmvSamplingPlugin',
            mlvl_feats1,
            mlvl_feats2,
            mlvl_feats3,
            mlvl_feats4,
    ...
    def forward(g,
                ...
                scale_weights):
        """run forward."""
        x = msmv_sampling_trt([mlvl_feats1, mlvl_feats2, mlvl_feats3, mlvl_feats4], sampling_locations, scale_weights)
        return x
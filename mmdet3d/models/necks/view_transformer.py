...
from mmdet3d.ops.bev_pool_v2.bev_pool import bev_pool_v2, TRTBEVPoolv2, TRTBEVPoolv2WithZ
...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor_trt(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
                       bda, inverse_post_rots, inverse_cam2imgs):
        """Calculate the locations of the frustum points in the lidar
        coordinate system.
        ...
        # points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3)\
        #     .matmul(points.unsqueeze(-1))
        points = inverse_post_rots.view(B, N, 1, 1, 1, 3, 3)\
            .matmul(points.unsqueeze(-1))
        ...
        # print(sensor2ego)
        # combine = sensor2ego[:,:,:3,:3].matmul(torch.inverse(cam2imgs))
        combine = sensor2ego[:,:,:3,:3].matmul(inverse_cam2imgs)
        points = combine.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        points += sensor2ego[:,:,:3, 3].view(B, N, 1, 1, 1, 3)
        points = bda.view(B, 1, 1, 1, 1, 3,
                          3).matmul(points.unsqueeze(-1)).squeeze(-1)
    ...
    def voxel_pooling_v2_trt(self, coor, depth, feat):
        ranks_bev, ranks_depth, ranks_feat, \
            interval_starts, interval_lengths = \
            self.voxel_pooling_prepare_v2(coor)
        if ranks_feat is None:
        ...
        #                        bev_feat_shape, interval_starts,
        #                        interval_lengths)
        bev_feat = TRTBEVPoolv2WithZ.apply(depth, feat,
                                    ranks_depth, ranks_feat, ranks_bev,
                                    interval_starts, interval_lengths, 
                                    int(200), int(200), int(16)) # change to bev shape
    ...
    def view_transform_core_trt(self, input, depth, tran_feat):
        B, N, C, H, W = input[0].shape
        ...
        if self.accelerate:
            ...
            coor = self.get_lidar_coor_trt(*input[1:9])
            bev_feat = self.voxel_pooling_v2_trt(
                coor, depth.view(N, self.D, H, W),
                tran_feat.view(N, self.out_channels, H, W))
        # print(self.D)
    ...
    def view_transform_trt(self, input, depth, tran_feat):
        if self.accelerate:
            self.pre_compute(input)
        return self.view_transform_core_trt(input, depth, tran_feat)
...
class DepthNet(nn.Module):
    ...
    def gen_grid_trt(self, metas, B, N, D, H, W, hi, wi):
        frustum = metas['frustum']
        points = frustum - metas['post_trans'].view(B, N, 1, 1, 1, 3)
        # points = torch.inverse(metas['post_rots']).view(B, N, 1, 1, 1, 3, 3) \
        #     .matmul(points.unsqueeze(-1))
        points = metas['inverse_post_rots'].view(B, N, 1, 1, 1, 3, 3) \
            .matmul(points.unsqueeze(-1))
        points = torch.cat(
            (points[..., :2, :] * points[..., 2:3, :], points[..., 2:3, :]), 5)
        ...
        trans = metas['k2s_sensor'][:, :, :3, 3].contiguous()
        # combine = rots.matmul(torch.inverse(metas['intrins']))
        combine = rots.matmul(metas['inverse_intrins'])
    ...
    def calculate_cost_volumn(self, metas, flag=False):
        prev, curr = metas['cv_feat_list']
        group_size = 4
        _, c, hf, wf = curr.shape
        hi, wi = hf * 4, wf * 4
        if flag:
            assert 'B' in metas and 'N' in metas
            B = metas['B']
            N = metas['N']
        else:
    ...
    def calculate_cost_volumn_henetpp(self, metas):
        prev, curr = metas['cv_feat_list']
        group_size = 4
        _, c, hf, wf = curr.shape
        c, hf, wf = int(c), int(hf), int(wf)
        hi, wi = hf * 4, wf * 4
        ...
        # metas['frustum'].shape = 88, 96, 176, 3
        # D, H, W = 88, 96, 176
        ...
        curr = curr.view(B * N, -1, H, W)
        cost_volumn = 0
        for fid in range(int(curr.shape[1]) // group_size):
            prev_curr = prev[:, fid * group_size:(fid + 1) * group_size, ...]
            wrap_prev = F.grid_sample(prev_curr, grid,
                                      align_corners=True,
                                      padding_mode='zeros')
    ...
    def calculate_cost_volumn_henetpp_bev(self, metas):
        prev, curr = metas['cv_feat_list']
        # group_size = 4
        group_size = 128
        ...
        # prev.shape = 6, 256, 96, 176
        # curr.shape = 6, 256, 96, 176
        cost_volumn = 0
        # process in group wise to save memory
        # curr.shape[1] = 256
        for fid in range(256 // group_size):
            prev_curr = prev[:, fid * group_size:(fid + 1) * group_size, ...]
            wrap_prev = F.grid_sample(prev_curr, grid,
                                      align_corners=True,
                                      padding_mode='zeros')
            curr_tmp = curr[:, fid * group_size:(fid + 1) * group_size, ...]
            # curr_tmp.shape = 6, 4, 96, 176
            # wrap_prev.shape = 6, 4, 8448, 176
            cost_volumn_tmp = curr_tmp.view(6, 128, 1, 96, 176) - \
                              wrap_prev.view(6, 128, 88, 96, 176)
            cost_volumn_tmp = cost_volumn_tmp.abs().sum(dim=1)
            cost_volumn += cost_volumn_tmp

        # Error Code 4: Internal Error ((Unnamed Layer* 167) [PluginV2DynamicExt]_output: tensor volume exceeds (2^31)-1, dimensions are [6,256,8448,176])
        # cannot grid_sample at once, need to split into 2 groups
        # prev_curr = prev
        # wrap_prev = F.grid_sample(prev_curr, grid,
        #                             align_corners=True,
        #                             padding_mode='zeros')
        ...
        # # curr_tmp.shape = 6, 4, 96, 176
        # # wrap_prev.shape = 6, 4, 8448, 176
        # cost_volumn_tmp = curr_tmp.view(6, 256, 1, 96, 176) - \
        #                     wrap_prev.view(6, 256, 88, 96, 176)
        # cost_volumn_tmp = cost_volumn_tmp.abs().sum(dim=1)
        # cost_volumn += cost_volumn_tmp

        # self.bias != 0
        invalid = wrap_prev[:, 0, ...].view(6, 88, 96, 176) == 0
        cost_volumn[invalid] = cost_volumn[invalid] + self.bias
        cost_volumn = - cost_volumn
        cost_volumn = cost_volumn.softmax(dim=1)

        # cost_volumn.shape = 6, 88, 96, 176
        return cost_volumn

    def calculate_cost_volumn_trt(self, metas):
        prev, curr = metas['cv_feat_list']
        group_size = 4
        _, c, hf, wf = curr.shape
        hi, wi = hf * 4, wf * 4
        B, N, _ = metas['post_trans'].shape
        D, H, W, _ = metas['frustum'].shape
        grid = self.gen_grid_trt(metas, B, N, D, H, W, hi, wi).to(curr.dtype)
    ...
    def forward(self, x, mlp_input, stereo_metas=None, flag=False):
        ...
        if not stereo_metas is None:
            if stereo_metas['cv_feat_list'][0] is None:
                ...
                with torch.no_grad():
                    if flag:
                        assert 'grid' in stereo_metas
                        cost_volumn = self.calculate_cost_volumn(stereo_metas, flag=True)
                    else:
    ...
    def forward_henetpp(self, x, mlp_input, stereo_metas=None):
        # mlp_input.shape = 1x6x27
        mlp_input = self.bn(mlp_input.view(6, 27))
        x = self.reduce_conv(x)
        ...
        # stereo_metas is not None
        # stereo_metas['cv_feat_list'][0] is not None
        with torch.no_grad():
            assert 'grid' in stereo_metas
            cost_volumn = self.calculate_cost_volumn_henetpp(stereo_metas)
    ...
    def forward_henetpp_bev(self, x, mlp_input, stereo_metas=None):
        # mlp_input.shape = 1x6x27
        mlp_input = self.bn(mlp_input.view(6, 27))
        x = self.reduce_conv(x)
        ...
        # with torch.no_grad():
        #     assert 'grid' in stereo_metas
        #     cost_volumn = self.calculate_cost_volumn_henetpp_bev(stereo_metas)
        ...
        # cost_volumn = self.cost_volumn_net(self.cost_volumn)
        # depth = torch.cat([depth, cost_volumn], dim=1)
    ...
    def forward_trt(self, x, mlp_input, stereo_metas=None):
        mlp_input = self.bn(mlp_input.reshape(-1, mlp_input.shape[-1]))
        x = self.reduce_conv(x)
        context_se = self.context_mlp(mlp_input)[..., None, None]
        context = self.context_se(x, context_se)
        ...
        if not stereo_metas is None:
            if stereo_metas['cv_feat_list'][0] is None:
                ...
                with torch.no_grad():
                    cost_volumn = self.calculate_cost_volumn_trt(stereo_metas)
            cost_volumn = self.cost_volumn_net(cost_volumn)
            depth = torch.cat([depth, cost_volumn], dim=1)
        if self.with_cp:
            depth = checkpoint(self.depth_conv, depth)
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def forward_trt(self, input, stereo_metas=None):
        (x, rots, trans, intrins, post_rots, post_trans, bda,
        inverse_post_rot, inverse_intrin, mlp_input) = input[:10]

        B, N, C, H, W = x.shape
        x = x.view(B * N, C, H, W)

        x = self.depth_net.forward_trt(x, mlp_input, stereo_metas)
        depth_digit = x[:, :self.D, ...]
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        bev_feat, depth = self.view_transform_trt(input, depth, tran_feat)
        return bev_feat, depth

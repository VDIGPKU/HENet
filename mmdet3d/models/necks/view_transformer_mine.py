...
# from mmdet3d.utils import set_up_deterministic_seed
...
class ViewTransformerLiftSplatShoot(BaseModule):
    def __init__(self, grid_config=None, data_config=None,
                 ...
                 accelerate=True, **kwargs):
        # FIXME change back!
        # set_up_deterministic_seed(0)
    ...
    def voxel_pooling(self, geom_feats, x):
        ...
        # flatten indices
        # TODO: figure out how camera coordinate frame change to bev coordinate frame (hint: intrinsic matrix)
        ...
        # shouldn't it really be griddify (B x C x Z x Y x X)? since nx[1] is really ny and nx[0] is nx
    ...
    def voxel_pooling_accelerated(self, geom_feats, x):
        B, N, D, H, W, C = x.shape
        Nprime = B * N * D * H * W
        nx = self.nx.to(torch.long)
        # flatten x
        ...
        unq, unq_inv = torch.unique(ranks, return_inverse=True, dim=0)
        unq = unq.long()

        # x = torch_scatter.scatter_sum(x, unq_inv, dim=0)

        canvas_size = int(B * nx[2] * nx[1] * nx[0])
        ...
        batch_canvas[unq] = x
        # batch_canvas = torch.cat(
            # batch_canvas.view(nx[0], nx[1], nx[2], B, C).permute(3, 4, 2, 1, 0).contiguous().unbind(dim=2), 1)
        # pay extra attention to the dim here!
        # batch_canvas = batch_canvas.view(B, nx[2], nx[1], nx[0], C)
        # batch_canvas = batch_canvas.permute(0, 4, 1, 2, 3).contiguous()
        batch_canvas = batch_canvas.view(nx[0], nx[1], nx[2], B, C)
        batch_canvas = batch_canvas.permute(3, 4, 2, 1, 0)


        batch_canvas = torch.cat(batch_canvas.unbind(dim=2), 1)

...
class LiftSplatShoot(nn.Module):
    ...
    def get_geometry(self, rots, trans, post_rots=None, post_trans=None,extra_rots=None,extra_trans=None,img_metas=None):
        ...
        if 'img_aug_matrix' in img_metas[0]:
            post_rots = torch.stack([i['img_aug_matrix'][..., :3, :3] for i in img_metas], dim=0).to(rots)
            post_trans = torch.stack([i['img_aug_matrix'][..., :3, 3] for i in img_metas], dim=0).to(rots)
        else:
            post_rots = None
            post_trans = None
    ...
    def bev_pool(self, geom_feats, x):
        from mmdet3d.ops.bev_pool.bev_pool_op import bev_pool as bev_pool_op
        B, N, D, H, W, C = x.shape
        Nprime = B * N * D * H * W

...
class IDAUpsample(BaseModule):
    ...
    def forward(self, mlvl_features, start_level, end_level):
        ...
        """
        # print(len(mlvl_features))
        # for tmp in mlvl_features:
        #     print(tmp.shape)
        for i in range(start_level, end_level - 1):
            upsample = self.ups[i - start_level]
            project = self.projs[i - start_level]
            # print(i, mlvl_features[i + 1].shape)
...
class DLANeck(BaseModule):
    ...
    def __init__(self,
        ...
        self.ida_up = IDAUpsample(
            ...
            in_channels[self.start_level+1],
            in_channels[self.start_level+1:self.end_level+1],
    ...
    def forward(self, x):
        ...
        for i in range(self.end_level - self.start_level):
            outs.append(mlvl_features[i+1].clone())
        self.ida_up(outs, 0, len(outs))
        # for tmp in outs:
            # print(tmp.shape)

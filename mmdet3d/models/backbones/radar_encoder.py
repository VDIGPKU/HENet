...
class RadarFeatureNet(nn.Module):
    ...
    #     coors_1 = coors[:, 1].to(dtype).unsqueeze(1).repeat(1, 10)
    #     f_center[:, :, 0] = features[:, :, 0] - (
    #         coors_1 * self.vx + self.x_offset
    #     )

    #     coors_2 = coors[:, 2].to(dtype).unsqueeze(1).repeat(1, 10)
    #     f_center[:, :, 1] = features[:, :, 1] - (
    #         coors_2 * self.vy + self.y_offset
    #     )
    ...
    #     features_1 = (features[:, :, 0:1] - self.pc_range[0]) / (self.pc_range[3] - self.pc_range[0])
    #     features_2 = (features[:, :, 1:2] - self.pc_range[1]) / (self.pc_range[4] - self.pc_range[1])
    #     features_3 = (features[:, :, 2:3] - self.pc_range[2]) / (self.pc_range[5] - self.pc_range[2])
    #     features_4 = features[:, :, 3:]
    #     features = torch.cat([features_1, features_2, features_3, features_4], dim=-1)
...
@BACKBONES.register_module()
class RadarFeatureNetV2(nn.Module):
    def __init__(
        self,
        in_channels=4,
        feat_channels=(64,),
    ...
    def forward(self, features, num_voxels, coors):
        ...
        if self.use_z:
            ...
            features_mean[:, :, 0] = features[:, :, 0] - features[:, :, 0].mean(dim=1).unsqueeze(1)
            features_mean[:, :, 1] = features[:, :, 1] - features[:, :, 1].mean(dim=1).unsqueeze(1)
    ...
    def forward_trt(self, features, num_voxels, coors):
        ...
        features_mean = torch.zeros_like(features[:, :, :2])
        # print(features.shape, mask.shape, features[:, :, 0].sum(dim=1).shape, mask.sum(dim=1).shape)
        # tmp = ((features[:, :, 0] * mask.squeeze()).sum(dim=1) / mask.squeeze().sum(dim=1)).unsqueeze(1)
        # print(mask[0], tmp[0])
        ...
        # print(features_mean[0], features[0], f_center[0])
        # print(features_mean[1], features[1], f_center[1])

...
from mmdet3d.core import (circle_nms, oval_nms, draw_heatmap_gaussian, gaussian_radius,
                          xywhr2xyxyr)
...
class CenterHead(BaseModule):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        # feature_map_size = grid_size[:2] // self.train_cfg['out_size_factor']  # <- float2int bug
        feature_map_size = torch.round(grid_size[:2] / self.train_cfg['out_size_factor'])
        ...
        for idx, task_head in enumerate(self.task_heads):
            heatmap = gt_bboxes_3d.new_zeros(
                (len(self.class_names[idx]), int(feature_map_size[1]),
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if nms_type == 'circle' or nms_type == 'oval':
                ret_task = []
        ...
        for i in range(num_samples):
            ...
            if False:
                ...
                plt.savefig("/home/wangxinhao/vis/pred/pred"+datetime.now().strftime('%H:%M:%S')+'--scene='+img_metas[0]['scene_token']+'--idx='+img_metas[0]['sample_idx']+".png")
                print('\n******** END PRINT GT**********\n')
                # exit(0)
        return ret_list

    def get_bboxes_trt(self, preds_dicts, rescale=False):
        """Generate bboxes from bbox head predictions.
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if 'vel' in preds_dict[0]:
                ...
                batch_vel = None
            temp = self.bbox_coder.decode_trt(
                batch_heatmap,
                batch_rots,
                batch_rotc,
                batch_hei,
                ...
                reg=batch_reg,
                task_id=task_id)
            per_batch_reg_preds = [box['bboxes'] for box in temp]
            per_batch_cls_preds = [box['scores'] for box in temp]
            per_batch_cls_labels = [box['labels'] for box in temp]
            per_batch_reg_preds = torch.stack(per_batch_reg_preds, dim=0) # bs x N x 9
            per_batch_cls_preds = torch.stack(per_batch_cls_preds, dim=0) # bs x N
            per_batch_cls_labels = torch.stack(per_batch_cls_labels, dim=0) # bs x N
            batch_reg_preds.append(per_batch_reg_preds)
            batch_cls_preds.append(per_batch_cls_preds)
            batch_cls_labels.append(per_batch_cls_labels)

        return batch_reg_preds, batch_cls_preds, batch_cls_labels

    def get_bbox_afterwards(self, total_reg_preds, total_cls_preds, total_cls_labels, img_metas):
        nms_type = self.test_cfg.get('nms_type')
        rets = []
        total_res = total_reg_preds.shape[1]
        per_res = 500
        final_box_preds_list = []
        final_scores_list = []
        final_preds_list = []
        for i in range(total_res // per_res):
            final_box_preds_list.append(total_reg_preds[:, i * per_res:(i + 1) * per_res, :])
            final_scores_list.append(total_cls_preds[:, i * per_res:(i + 1) * per_res])
            final_preds_list.append(total_cls_labels[:, i * per_res:(i + 1) * per_res])

        for task_id in range(len(final_box_preds_list)):
            batch_size = final_box_preds_list[task_id].shape[0]
            temp = self.bbox_coder.decode_trt_afterwards(
                final_box_preds_list[task_id],
                final_scores_list[task_id],
                final_preds_list[task_id],
                task_id=task_id)
            batch_reg_preds = [box['bboxes'] for box in temp]
            batch_cls_preds = [box['scores'] for box in temp]
            batch_cls_labels = [box['labels'] for box in temp]

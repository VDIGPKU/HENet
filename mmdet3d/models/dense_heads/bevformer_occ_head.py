...
class BEVFormerOccHead(BaseModule):
    ...
    def get_occ(self, preds_dicts, img_metas, rescale=False):
        ...
        #occ_score_np = occ_score.squeeze().detach().cpu().numpy()
        #occ_score_np[occ_score_np>0.5]=1
        #occ_score_np[occ_score_np<=0.5]=0
        #name = img_metas[0]['pts_filename'].split('/')[-1].split('.')[0]
        #save_path = os.path.join('/workspace/codes/occupancy/occupancy/CVPR2023-3D-Occupancy-Prediction/2_class_sweep2', name+'_pred.npz')
        #img_name = img_metas[0]['filename']
        #pt_name = img_metas[0]['pts_filename']
        #np.savez_compressed(save_path,pred=occ_score_np,imgName=img_name,ptName=pt_name)

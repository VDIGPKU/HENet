...
class SparseBEVHead(DETRHead):
    def __init__(self,
        ...
        self.return_bbox_feat = return_bbox_feat
    ...
    def forward(self, mlvl_feats, img_metas, **kwargs):
        ...
        cls_scores, bbox_preds, bbox_feats = self.transformer(
            query_bbox,
            query_feat,
            mlvl_feats,
            attn_mask=attn_mask,
        ...
        if self.return_bbox_feat:
            return outs, bbox_feats
        else:
            return outs

    def forward_trt(self, mlvl_feats, img_metas, **kwargs):
        query_bbox = self.init_query_bbox.weight.clone()  # [Q, 10]
        #query_bbox[..., :3] = query_bbox[..., :3].sigmoid()
        ...
        cls_scores, bbox_preds = self.transformer.forward_trt(
            query_bbox,
            query_feat,
            mlvl_feats,
            attn_mask=attn_mask,
        ...
        return cls_scores, bbox_preds  # mask_dict == None

    def forward_trt_debug(self, mlvl_feats, img_metas, **kwargs):
        query_bbox = self.init_query_bbox.weight.clone()  # [Q, 10]
        #query_bbox[..., :3] = query_bbox[..., :3].sigmoid()
        ...
        cls_scores, bbox_preds = self.transformer.forward_trt_debug(
            query_bbox,
            query_feat,
            mlvl_feats,
            attn_mask=attn_mask,
    ...
    def dn_loss_single(self,
        ...
        bbox_weights = bbox_weights * bbox_weights.new_tensor(self.code_weights)
    ...
    def _get_target_single(self,
        ...
        code_weights = bbox_pred.new_tensor(self.code_weights)
        assign_result = self.assigner.assign(bbox_pred, cls_score, gt_bboxes, gt_labels, gt_bboxes_ignore, code_weights, True)
...
@HEADS.register_module()
class SparseBEVHeadTRT(SparseBEVHead):
    def __init__(self,
                 **kwargs):

        super(SparseBEVHeadTRT, self).__init__(**kwargs)
        self.training = False

        device = self.init_query_bbox.weight.device

        self.query_bbox = self.init_query_bbox.weight.clone() # [Q, 10]
        #query_bbox[..., :3] = query_bbox[..., :3].sigmoid()

        # query denoising
        B = 1
        query_bbox, query_feat, self.attn_mask, self.mask_dict = self.prepare_for_dn_input(B, self.query_bbox, self.label_enc, None)
        self.query_bbox = query_bbox.clone().detach() # remove grad
        self.query_feat = query_feat.clone().detach() # remove grad
        # attn_mask = None, mask_dict = None

...
class SparseBEVHead_rc(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_rc, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)

        # a bug: code_weights will load from ckpt, w_vx, w_vy still equal to 1
    ...
    def forward_trt(self, mlvl_feats, radar_feats, img_metas):
        query_bbox = self.init_query_bbox.weight.clone()  # [Q, 10]
        #query_bbox[..., :3] = query_bbox[..., :3].sigmoid()
    ...
    def prepare_for_dn_input(self, batch_size, init_query_bbox, label_enc, img_metas):
        ...
        if self.training and self.dn_enabled:
            ...
            known_num = [sum(k) for k in known] # 奇怪的bug
            ...
            # known_bid = batch_idx.expand(self.dn_group_num, -1).reshape(-1).clone()

...
class Decoder(nn.Module):
    def __init__(self, dim, blocks, out_dim,residual=True, factor=2):  # Decoder(self.embed_dims,self.blocks,1) blocks=[128,128,64]
        super().__init__()
...
class SparseBEVHead_rc_seg(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        self.num_pred = 1 #new
        super(SparseBEVHead_rc_seg, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        #self.connected_layer = nn.Linear(in_features = 310, out_features = 625)
    ...
    def _init_layers(self):
        ...
        grid_size = int(math.sqrt(self.num_query)) #dn
    ...
    def forward(self, mlvl_feats, radar_feats, img_metas):
        ...
        outs_dec_lane, _ = self.transformer(    #new outs_dec_lane [num_layers, bs, num_query, dim] #query_feat torch.Size([8, 1490, 256])
            query_bbox,
            query_feat,
            mlvl_feats,
            radar_feats,
        ...
        outs_dec_lane = torch.nan_to_num(outs_dec_lane) #new outs_dec_lane
        lane_queries=outs_dec_lane #new outs_dec_lane
        outputs_lanes=[]
        # if mask_dict is not None and mask_dict['pad_size'] > 0:  # if using query denoising
        #     print(mask_dict['pad_size'])
        #     denoisedq = outs_dec_lane[:, :mask_dict['pad_size'], :]

        #     denoisedq = self.connected_layer(denoisedq)
        #     lane_denoisedq = denoisedq.view(B, 25, 25,-1).permute(0,3,1,2) #??
        #     mask_dict['output_known_lbs_bboxes'] = lane_denoisedq

        #     outs_dec_lane = outs_dec_lane[:, mask_dict['pad_size']:, :]
        #     B, Q, _  =outs_dec_lane.shape #torch.Size([1, 1210, 256]) 935
        #     grid_size = int(math.sqrt(Q))
        #     assert grid_size * grid_size == Q
        #     # print('grid_size',grid_size)
        #     # print("Q",Q) 
        #     lane_queries_lvl=lane_queries.view(B, grid_size, grid_size,-1).permute(0,3,1,2) #??
        #     outputs_dri=self.lane_branches_dri[0](lane_queries_lvl)
        #     outputs_lan=self.lane_branches_lan[0](lane_queries_lvl)
        #     outputs_vie=self.lane_branches_vie[0](lane_queries_lvl)
        #     # print("outputs_dri.shape",outputs_dri.shape) #outputs_dri.shape torch.Size([1, 1, 400, 400])
        #     # exit(0)

...
from .sparsebev_sampling import sampling_4d, sampling_4d_trt, make_sample_points, sampling_4d_trt_debug
...
class SparseBEVTransformer(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10,
                 code_size=10, pc_range=[], init_cfg=None, **kwargs):
        ...
        self.decoder = SparseBEVTransformerDecoder(embed_dims, num_frames, num_points, num_layers,
                                                   num_levels, num_classes, code_size, pc_range=pc_range,
                                                   **kwargs)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs):
        cls_scores, bbox_preds, bbox_feats = self.decoder(query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs)
    ...
    def forward_trt(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs):
        cls_scores, bbox_preds = self.decoder.forward_trt(query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs)
    ...
    def forward_trt_debug(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs):
        sampled_feat = self.decoder.forward_trt_debug(query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs)
        
        return sampled_feat
    
        cls_scores, bbox_preds = self.decoder.forward_trt_debug(query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs)
...
class SparseBEVTransformerDecoder(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10,
        ...
        self.decoder_layer = SparseBEVTransformerDecoderLayer(
            embed_dims, num_frames, num_points, num_levels, num_classes, code_size, pc_range=pc_range, **kwargs
        )
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, img_feats_lt=None, img_metas_lt=None, **kwargs):
        ...
        # openad
        # timestamps = np.reshape(timestamps, [query_bbox.shape[0], -1, 1])
        ...
        if img_metas_lt is not None:
            timestamps = np.array([m['img_timestamp'] for m in img_metas_lt], dtype=np.float64)
            timestamps = np.reshape(timestamps, [query_bbox.shape[0], -1, 6])
            time_diff = timestamps[:, :1, :] - timestamps
            time_diff = np.mean(time_diff, axis=-1).astype(np.float32)  # [B, F]
            time_diff = torch.from_numpy(time_diff).to(query_bbox.device)  # [B, F]
            img_metas_lt[0]['time_diff'] = time_diff
        ...
        if img_metas_lt is not None:
            lidar2img_lt = np.asarray([m['lidar2img'] for m in img_metas_lt]).astype(np.float32)
            lidar2img_lt = torch.from_numpy(lidar2img_lt).to(query_bbox.device)  # [B, N, 4, 4]
            img_metas_lt[0]['lidar2img'] = lidar2img_lt
        ...
        if img_feats_lt is not None:
            for lvl, feat in enumerate(img_feats_lt):
                B, TN, GC, H, W = feat.shape  # [B, TN, GC, H, W]
                N, T, G, C = 6, TN // 6, 4, GC // 4
                feat = feat.reshape(B, T, N, G, C, H, W)
                ...
                img_feats_lt[lvl] = feat.contiguous()
        ...
        for i in range(self.num_layers):
            ...
            query_feat, cls_score, bbox_pred = self.decoder_layer(
                ...
                mlvl_feats_lt=img_feats_lt, img_metas_lt=img_metas_lt
            )
    ...
    def forward_trt(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs):
        ...
        # bbox_pred = query_bbox
        # for i in range(self.num_layers):
        #     DUMP.stage_count = i

        #     query_feat, cls_score, bbox_pred = self.decoder_layer.forward_trt(
        #         bbox_pred, query_feat, mlvl_feats, attn_mask, img_metas)
    ...
    def forward_trt_debug(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **kwargs):
        ...
        for i in range(self.num_layers):
            DUMP.stage_count = i
            sampled_feat = self.decoder_layer.forward_trt_debug(
                query_bbox, query_feat, mlvl_feats, attn_mask, img_metas)

            return sampled_feat

            query_feat, cls_score, bbox_pred = self.decoder_layer.forward_trt_debug(
                query_bbox, query_feat, mlvl_feats, attn_mask, img_metas)
...
class SparseBEVTransformerDecoderLayer(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_levels=4, num_classes=10, code_size=10,
                 num_cls_fcs=2, num_reg_fcs=2, pc_range=[], init_cfg=None, sampling_lt=None, **kwargs):
        ...
        if sampling_lt is None:
            self.sampling_lt = None
        else:
            self.sampling_lt = SparseBEVSampling(
                sampling_lt['embed_dims'],
                num_frames=sampling_lt['num_frames'],
                num_groups=4,
                num_points=sampling_lt['num_points'],
                num_levels=sampling_lt['num_levels'],
                pc_range=pc_range
            )
            self.mixing_lt = AdaptiveMixing(
                in_dim=sampling_lt['embed_dims'],
                in_points=sampling_lt['num_points'] * sampling_lt['num_frames'],
                n_groups=4,
                out_points=128,
            )
            self.norm2_lt = nn.LayerNorm(sampling_lt['embed_dims'])
            self.FFN_mixlt = nn.Linear(embed_dims + sampling_lt['embed_dims'], embed_dims)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, mlvl_feats_lt=None, img_metas_lt=None):
        """
        ...
        if self.sampling_lt is not None:
            sampled_feat_lt = self.sampling_lt(query_bbox, query_feat, mlvl_feats_lt, img_metas_lt)

        query_feat_st = self.norm2(self.mixing(sampled_feat, query_feat))
        if self.sampling_lt is not None:
            query_feat_lt = self.norm2_lt(self.mixing_lt(sampled_feat_lt, query_feat))
            query_feat = self.FFN_mixlt(torch.cat([query_feat_st, query_feat_lt], dim=-1))
        else:
            query_feat = query_feat_st
    ...
    def forward_trt(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas):
        """
        query_bbox: [B, Q, 10] [cx, cy, cz, w, h, d, rot.sin, rot.cos, vx, vy]
        """
        query_pos = self.position_encoder(query_bbox[..., :3])
        ...
        query_feat = self.norm1(self.self_attn(query_bbox, query_feat, attn_mask))
        sampled_feat = self.sampling.forward_trt(query_bbox, query_feat, mlvl_feats, img_metas)
    ...
    def forward_trt_debug(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas):
        """
        query_bbox: [B, Q, 10] [cx, cy, cz, w, h, d, rot.sin, rot.cos, vx, vy]
        """
        query_pos = self.position_encoder(query_bbox[..., :3])
        ...
        query_feat = self.norm1(self.self_attn(query_bbox, query_feat, attn_mask))
        sampled_feat = self.sampling.forward_trt_debug(query_bbox, query_feat, mlvl_feats, img_metas)
        return sampled_feat
...
class SparseBEVSampling(BaseModule):
    ...
    def inner_forward_trt(self, query_bbox, query_feat, mlvl_feats, img_metas):
        '''
        query_bbox: [B, Q, 10]
        query_feat: [B, Q, C]
        '''
    ...
    def forward_trt(self, query_bbox, query_feat, mlvl_feats, img_metas):
        if self.training and query_feat.requires_grad:
            return cp(self.inner_forward_trt, query_bbox, query_feat, mlvl_feats, img_metas, use_reentrant=False)
        else:
            return self.inner_forward_trt(query_bbox, query_feat, mlvl_feats, img_metas)

    def inner_forward_trt_debug(self, query_bbox, query_feat, mlvl_feats, img_metas):
        '''
        query_bbox: [B, Q, 10]
        query_feat: [B, Q, C]
        '''
        ...
        scale_weights = scale_weights.expand(B, Q, self.num_groups, self.num_frames, self.num_points, self.num_levels)
        # sampling
        sampled_feats = sampling_4d_trt_debug(
            sampling_points,
            mlvl_feats,
            scale_weights,
            img_metas[0]['lidar2img'],
    ...
    def forward_trt_debug(self, query_bbox, query_feat, mlvl_feats, img_metas):
        if self.training and query_feat.requires_grad:
            return cp(self.inner_forward_trt_debug, query_bbox, query_feat, mlvl_feats, img_metas, use_reentrant=False)
        else:
            return self.inner_forward_trt_debug(query_bbox, query_feat, mlvl_feats, img_metas)
...
class AdaptiveMixing(nn.Module):
    ...
    def inner_forward_trt(self, x, query):
        ...
        # int(out.size(-2)), int(out.size(-1)) = 32, 64
        weight_value = torch.ones([int(out.size(-2)), int(out.size(-1))], dtype=out.dtype, device=out.device)
        bias_value = torch.zeros([int(out.size(-2)), int(out.size(-1))], dtype=out.dtype, device=out.device)
        ...
        '''adaptive point mixing'''
        out = torch.matmul(S, out)  # implicitly transpose and matmul
        # int(out.size(-2)), int(out.size(-1)) = 128, 64
        weight_value = torch.ones([int(out.size(-2)), int(out.size(-1))], dtype=out.dtype, device=out.device)
        bias_value = torch.zeros([int(out.size(-2)), int(out.size(-1))], dtype=out.dtype, device=out.device)
        out = F.layer_norm(out, [int(out.size(-2)), int(out.size(-1))], weight=weight_value, bias=bias_value)
        # out = F.layer_norm(out, [int(out.size(-2)), int(out.size(-1))])

...
from .sparsebev_sampling import sampling_4d, make_sample_points, sampling_bev
from .checkpoint import checkpoint as cp
from mmdet3d.ops.csrc.wrapper import MSMV_CUDA
from mmcv.cnn import ConvModule, xavier_init, normal_init

@TRANSFORMER.register_module()
class SparseBEVTransformer_rc_changan(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10, code_size=10, pc_range=[], init_cfg=None):
        ...
        super(SparseBEVTransformer_rc_changan, self).__init__(init_cfg=init_cfg)
...
class SparseBEVSampling(BaseModule):
    ...
    def __init__(self, embed_dims=256, num_frames=4, num_groups=4, num_points=8, num_levels=4, pc_range=[], init_cfg=None, radar_dim=48):
        super().__init__(init_cfg)
        ...
        self.alpha = nn.Linear(radar_dim, 64)
    ...
    def init_weights(self):
        ...
        nn.init.uniform_(self.alpha.bias, -0.1, 0.1)
        print('init alpha to zero!!!!')
    ...
    def inner_forward(self, query_bbox, query_feat, mlvl_feats, radar_feats, img_metas):
        ...
        # sampled_feats = torch.concat([sampled_feats, self.alpha * sampled_feats_bev], dim=-1)
        sampled_feats = sampled_feats + self.alpha(sampled_feats_bev)
...
class AdaptiveMixing(nn.Module):
    ...
    def __init__(self, in_dim, in_points, n_groups=1, query_dim=None, out_dim=None, out_points=None):
        ...
        self.in_dim = in_dim #in_dim +radar

...
class SparseBEV_rc_seg(MVXTwoStageDetector):
    def __init__(self,
                 data_aug=None,
                 stop_prev_grad=0,
        ...
        super(SparseBEV_rc_seg, self).__init__(**kwargs)
    ...
    def init_weights(self):
        ...
        super(SparseBEV_rc_seg, self).init_weights()
        if self.freeze_img:
    ...
    def forward_pts_train(self,
        ...
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs, maps] #maps
    ...
    def forward_train(self,
        ...
        losses = self.forward_pts_train(img_feats, radar_feats, maps, gt_bboxes_3d, gt_labels_3d, img_metas, gt_bboxes_ignore)

        return losses

    def forward_test(self, img_metas, gt_map, maps, img=None, radar=None, **kwargs): #gt_map, maps,
        for var, name in [(img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
        img = [img] if img is None else img
        return self.simple_test(img_metas[0], gt_map, img[0], maps, radar[0], **kwargs) #gt_map, maps,

    def simple_test_pts(self, x, radar_feats, img_metas, gt_map, maps, rescale=False):
        outs = self.pts_bbox_head(x, radar_feats, img_metas)
        #bbox_list = self.pts_bbox_head.get_bboxes(outs, img_metas[0], rescale=rescale)
        with torch.no_grad():
            
            lane_preds=outs['all_lane_preds'][0].squeeze(0)    #[B,N,H,W]
    ...
    def simple_test(self, img_metas, gt_map=None, img=None, maps=None, radar=None, rescale=False): # gt & maps
        world_size = get_dist_info()[1]
        if world_size == 1:  # online
            return self.simple_test_online(img_metas, gt_map, img, maps, radar, rescale)
        else:  # offline
            return self.simple_test_offline(img_metas, gt_map, img, maps, radar, rescale)

    def simple_test_offline(self, img_metas, gt_map=None, img=None, maps=None, radar=None, rescale=False):
        img_feats, radar_feats= self.extract_feat(img=img, img_metas=img_metas, radar=radar)

        bbox_list = [dict() for _ in range(len(img_metas))]
        ret_iou = self.simple_test_pts(img_feats, radar_feats, img_metas, gt_map, maps, rescale=rescale)
        for result_dict in bbox_list:
            result_dict['ret_iou'] = ret_iou

        return bbox_list

    def simple_test_online(self, img_metas, gt_map=None, img=None, maps=None, radar=None, rescale=False):

...
class Mobilenetv3(BaseDetector):

    def __init__(self, train_cfg=None, test_cfg=None, pretrained=None, **kwargs):
        super().__init__(**kwargs)
        self.model = MobileNetV3_Small(num_classes=1)

    def forward(self, img, gt_class=None, return_loss=True, **kwargs):
        if type(img) is list:
            img = img[0]
        if len(img.shape) == 3:
            img = img.unsqueeze(0)
        ...
        pred = (result > 0.5).float()
        if gt_class is None:
            return pred
        if return_loss:
            correct = (pred == gt_class).sum().item()
            acc = correct / len(gt_class)
            acc = torch.Tensor([acc]).to(result.device)
            loss_cls = F.binary_cross_entropy(result, gt_class.float())
            return dict(
                loss_cls=loss_cls,
                acc=acc,
            )

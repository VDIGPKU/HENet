...
@PIPELINES.register_module()
class GlobalRotScaleTrans_point(object):
    """Apply global rotation, scaling and translation to a 3D scene for radardataset.
    ...
    def _flip_bbox_points(self, input_dict):
        ...
        Returns:
            dict: Results after translation, 'points', 'pcd_trans'
                ...
                in the result dict.
        """
        flip_dx=input_dict['flip_dx']
        flip_dy=input_dict['flip_dy']
        if flip_dx:
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        Returns:
            dict: Results after rotation, 'points', 'pcd_rotation'
                ...
                in the result dict.
        """
        noise_rotation = input_dict['rotate_bda']#np.random.uniform(rotation[0], rotation[1]) #
        #print(noise_rotation)
        # if no bbox in input_dict, only rotate points
        #if len(input_dict['bbox3d_fields']) == 0:
        rot_mat_T = input_dict['points'].rotate(noise_rotation/180*math.pi) #radar
    ...
    def __call__(self, input_dict):
        ...
        self._rot_bbox_points(input_dict) #radar
...
class PointShuffle(object):
    ...
    def __call__(self, input_dict):
        ...
        if False:  # for debug vis
            ...
            points = input_dict['points']
            corner = input_dict['gt_bboxes_3d'].corners
...
class GlobalRotScaleTrans_radar(object):
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        # if len(input_dict['bbox3d_fields']) == 0:
        if (self.is_rad):
            rot_mat_T = input_dict['radar'].rotate(noise_rotation)  # radar
        else:
            rot_mat_T = input_dict['radar'].rotate(noise_rotation / 180 * math.pi)  # radar
        input_dict['pcd_rotation_radar'] = rot_mat_T
...
class PhotoMetricDistortionMultiViewImage:
    ...
    def __call__(self, results):
        ...
        for img in imgs:
            ...
            # random brightness
            if randomm.randint(2):
                delta = randomm.uniform(-self.brightness_delta,
                                    self.brightness_delta)
                img += delta

            # mode == 0 --> do random contrast first
            # mode == 1 --> do random contrast last
            mode = randomm.randint(2)
            if mode == 1:
                if randomm.randint(2):
                    alpha = randomm.uniform(self.contrast_lower,
                                        self.contrast_upper)
                    img *= alpha
            ...
            # random saturation
            if randomm.randint(2):
                img[..., 1] *= randomm.uniform(self.saturation_lower,
                                            self.saturation_upper)

            # random hue
            if randomm.randint(2):
                img[..., 0] += randomm.uniform(-self.hue_delta, self.hue_delta)
                img[..., 0][img[..., 0] > 360] -= 360
                img[..., 0][img[..., 0] < 0] += 360
...
class RandomTransformImage(object):
    def __init__(self, ida_aug_conf=None, training=True, suffix='', cut_num=0):
    ...
    def __call__(self, results):
        ...
        if self.suffix != '':
            results['img'+self.suffix] = results['img']
            results['img_timestamp'+self.suffix] = results['img_timestamp']
            results['filename'+self.suffix] = results['filename']
            results['lidar2img'+self.suffix] = results['lidar2img']
            results['img'] = results['img'][:6*self.cut_num]
            results['img_timestamp'] = results['img_timestamp'][:6*self.cut_num]
            results['filename'] = results['filename'][:6*self.cut_num]
            results['lidar2img'] = results['lidar2img'][:6*self.cut_num]

        if len(results['lidar2img'+self.suffix]) == len(results['img'+self.suffix]):
            for i in range(len(results['img'+self.suffix])):
                img = Image.fromarray(np.uint8(results['img'+self.suffix][i]))
                ...
                results['img'+self.suffix][i] = np.array(img).astype(np.uint8)
                results['lidar2img'+self.suffix][i] = ida_mat @ results['lidar2img'+self.suffix][i]

        elif len(results['img'+self.suffix]) == 6:
            for i in range(len(results['img'+self.suffix])):
                img = Image.fromarray(np.uint8(results['img'+self.suffix][i]))
            ...
            for i in range(len(results['lidar2img'+self.suffix])):
                results['lidar2img'+self.suffix][i] = ida_mat @ results['lidar2img'+self.suffix][i]

        else:
            raise ValueError()

        results['ori_shape'+self.suffix] = [img.shape for img in results['img'+self.suffix]]
        results['img_shape'+self.suffix] = [img.shape for img in results['img'+self.suffix]]
        results['pad_shape'+self.suffix] = [img.shape for img in results['img'+self.suffix]]
...
class GlobalRotScaleTransImage(object):
    ...
    def rotate_z(self, results, rot_angle):
        ...
        for view in range(len(results['lidar2img'])):
            for suffix in self.suffix_list:
                results['lidar2img'+suffix][view] = (torch.tensor(results['lidar2img'+suffix][view]).float() @ rot_mat_inv).numpy()
    ...
    def scale_xyz(self, results, scale_ratio):
        ...
        for view in range(len(results['lidar2img'])):
            for suffix in self.suffix_list:
                results['lidar2img'+suffix][view] = (torch.tensor(results['lidar2img'+suffix][view]).float() @ scale_mat_inv).numpy()
...
class DistillPreprocess3D():
    def __init__(self,
        ...
        self.transform = T.Compose([
                ...
                # no need of further operations, so totensor and normalize here
                T.ToTensor(),
                T.Normalize(mean=mean, std=std)
            ])

    def __call__(self, results):
        if self.distill_type == 'internvl':
            # no need to open image, we already did flip and rotate in PrepareImageInputs
            pixel_values_list = []
            imgs_distill = results['imgs_distill']
            for img in imgs_distill:
                imgs = self.dynamic_preprocess(
                            img,
                ...
                pixel_values = [self.transform(img) for img in imgs]
                if False:
                    import matplotlib.pyplot as plt
                    plt.axis('off')  # 可选，关闭坐标轴显示
                    for i, pixel_value in enumerate(imgs):
                        plt.imshow(pixel_value)
                        plt.savefig('pixel_value_' + str(i) + '.png', format='png')

                    exit(0)
                pixel_values = torch.stack(pixel_values)
                pixel_values_list.append(pixel_values)
            pixel_values_list = torch.stack(pixel_values_list)
            results['pixel_values'] = pixel_values_list
        else:
            assert False

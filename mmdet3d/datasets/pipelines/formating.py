...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'img_lt' in results:
            if isinstance(results['img_lt'], list):
                # process multiple imgs in single frame
                imgs = [img.transpose(2, 0, 1) for img in results['img_lt']]
                imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
                results['img_lt'] = DC(to_tensor(imgs), stack=True)
            else:
                img = np.ascontiguousarray(results['img_lt'].transpose(2, 0, 1))
                results['img_lt'] = DC(to_tensor(img), stack=True)
        for key in [
...
class Collect3D(object):
    ...
    def __init__(
        ...
        meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                   ...
                   'affine_aug','scene_token','P2','Trv2c','rect', #VOD
                   'lidar2ego', 'ego2global', 'cam_ego2global',
                   'transformation_3d_flow_radar','pcd_scale_factor_radar','pcd_rotation_radar',
                   'pcd_rotation_angle_radar','pcd_trans_radar', #add radar
                   'adj_gt_3d', 'img_file_paths'),
        second_meta_keys=None,
        second_meta_name=None):
        self.keys = keys
        self.meta_keys = meta_keys
        self.second_meta_keys = second_meta_keys
        self.second_meta_name = second_meta_name
    ...
    def __call__(self, results):
        ...
        if self.second_meta_keys and self.second_meta_name:
            sec_metas = {}
            for key in self.second_meta_keys:
                if key in results:
                    sec_metas[key.replace('_lt', '')] = results[key]
            data[self.second_meta_name] = DC(sec_metas, cpu_only=True)
...
class DefaultFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        if "radar" in results:
            if type(results['radar']) is not dict:
                results["radar"] = DC(results["radar"].tensor)   #radar tensor

...
class PlanningMetric():
    ...
    def get_birds_eye_view_label(
        ...
        gt_agent_boxes = gt_agent_boxes.cpu().numpy()
        ...
        for t in range(T):
            for i in range(agent_num):
                if gt_agent_fut_mask[i][t] == 1:
                    ...
                    # yaw_a = gt_agent_fut_yaw[i, t, 0]  # for mmdet3d<1.0 info
                    yaw_a = -gt_agent_fut_yaw[i, t, 0]  # for mmdet3d>1.0 info
                    param = [x_a,y_a,yaw_a,agent_length/2, agent_width/2]
    ...
    def evaluate_coll(
        ...
        segmentation = segmentation.to(device=trajs.device)
        ...
        for i in range(B):
            ...
            ti = torch.arange(n_future).to(device=trajs.device)
    ...
    def compute_L2_ade(self, trajs, gt_trajs):  # this is for ADE
        '''
    ...
    def compute_L2_fde(self, trajs, gt_trajs):  # this is for FDE, final displacement error
        '''
        trajs: torch.Tensor (n_future, 2)
        gt_trajs: torch.Tensor (n_future, 2)
        '''

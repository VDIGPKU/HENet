...
class CocoDataset_custom(CustomDataset):
    ...
    def evaluate(self,
        ...
        metrics = self.metrics  # ['bbox', 'segm']
        allowed_metrics = ['bbox', 'segm', 'proposal', 'proposal_fast', 'cls']
        for metric in metrics:
        ...
        if 'cls' in metrics:
            ...
            eval_results_1 = OrderedDict()
            # print(results[0])
            acc_list = [i[2] for i in results]
            eval_results_1["cls_acc"] = sum(acc_list)/len(acc_list)
            eval_results.update(eval_results_1)
        if len(metrics) > 0:
            result_files, tmp_dir = self.format_results(results, jsonfile_prefix)
            eval_results_2 = self.evaluate_det_segm(results, result_files, coco_gt,
                                                    metrics, logger, classwise,
                                                    proposal_nums, iou_thrs,
                                                    metric_items)
            eval_results.update(eval_results_2)

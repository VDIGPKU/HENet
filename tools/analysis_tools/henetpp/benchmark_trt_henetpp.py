...
'''
fpn only:
PYTHONPATH='.' python ./tools/analysis_tools/henetpp/benchmark_trt_henetpp.py \
configs/henetpp/rc_multitask.py \
work_dirs/quant_detsegVAD_640_bevmix_diffbev_symmetric/epoch_8.pth \
mmdeploy/henetpp/henetpp_sim.engine \
--samples 6019 
...
configs/henetpp/rc_multitask.py \
work_dirs/quant_detsegVAD_640_bevmix_diffbev_symmetric/epoch_8.pth \
mmdeploy/henetpp_v2/henetpp_v2_sim.engine \
--samples 6019 
'''
def parse_args():
...
def main():
    ...
    trt_model = TRTWrapper(args.engine,
                           ['occ_res', 'cls_scores', 'bbox_preds', 'feat_2d_ret', 'img_feats_curr_1', 'img_feats_curr_2', 'img_feats_curr_3', 'img_feats_curr_4'])
                            # ['bev_feats', 'bev_feats_1', 'bev_feats_2', 'bev_feats_3', 'bev_feats_4', 'bev_feats_5', 'bev_feats_6', 'bev_feats_7', 'bev_feats_8'])
    ...
    hyper_data_path = 'mmdeploy/henetpp_v2/henetpp_v2_hyper_data.pkl'
    ...
    sensor2egos = data_dict['sensor2egos'][0].to(device0).contiguous()
    # ego2globals = data_dict['ego2globals'][0].to(device0).contiguous()
    intrins = data_dict['intrins'][0].to(device0).contiguous()
    inverse_intrins = data_dict['inverse_intrins'][0].to(device0).contiguous()
    post_rots = data_dict['post_rots'][0].to(device0).contiguous()
    inverse_post_rots = data_dict['inverse_post_rots'][0].to(device0).contiguous()
    post_trans = data_dict['post_trans'][0].to(device0).contiguous()
    curr2adjsensor = data_dict['curr2adjsensor'][0].to(device0).contiguous()
    bda = data_dict['bda'][0].to(device0).contiguous()
    feat_prev = data_dict['feat_prev'][0].to(device0).contiguous()
    stereo_feat_prev = data_dict['stereo_feat_prev'][0].to(device0).contiguous()
    voxels = data_dict['voxels'][0].to(device0).contiguous()
    num_points = data_dict['num_points'][0].to(device0).contiguous()
    coors = data_dict['coors'][0].to(device0).contiguous()
    lidar2img = data_dict['lidar2img'][0].to(device0).float().contiguous()
    img_timestamp = data_dict['img_timestamp'][0].to(device0).contiguous()
    # len_img_filenames = data_dict['len_img_filenames'][0]
    feat_prev_1_sparse = data_dict['feat_prev_1_sparse'][0].to(device0).contiguous()
    feat_prev_2_sparse = data_dict['feat_prev_2_sparse'][0].to(device0).contiguous()
    feat_prev_3_sparse = data_dict['feat_prev_3_sparse'][0].to(device0).contiguous()
    feat_prev_4_sparse = data_dict['feat_prev_4_sparse'][0].to(device0).contiguous()
    ...
    for i in range(args.samples):
        with torch.no_grad():
            trt_output, elapsed = trt_model.forward(dict(
                                    ...
                                    intrins=intrins,
                                    inverse_intrins=inverse_intrins,
                                    post_rots=post_rots,
                                    inverse_post_rots=inverse_post_rots,
                                    post_trans=post_trans,
                                    bda=bda,
                                    curr2adjsensor=curr2adjsensor,

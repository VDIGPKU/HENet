...
fpn only:
henetpp_bev:
PYTHONPATH='.' python ./tools/analysis_tools/henetpp/benchmark_trt_henetpp_orin.py \
...
mmdeploy/ac_changan/changan_int8_fuse.engine \
--samples 6019
'''
def parse_args():
...
def main():
    ...
    # model = model.cuda()
    # model.pts_bbox_head.query_bbox = model.pts_bbox_head.query_bbox.cuda()
    # model.pts_bbox_head.query_feat = model.pts_bbox_head.query_feat.cuda()
    # model.img_view_transformer.cv_frustum = model.img_view_transformer.cv_frustum.cuda()
    # model.img_view_transformer.depth_net.cost_volumn = model.img_view_transformer.depth_net.cost_volumn.cuda()
    # model.eval()
    pure_inf_time = []
    ...
    hyper_data_path = args.engine.replace('.engine', '_data.pkl')

...
'''
henetpp_bev:
PYTHONPATH='.' python ./tools/analysis_tools/henetpp/benchmark_trt_henetpp_ac_1kf_orin.py \
configs/henetpp/changan_rc_multitask_small_res_deploy.py \
work_dirs/changan_rc_multitask_small_res_deploy/epoch_12.pth \
mmdeploy/ac_changan_1kf/changan_1kf_int8_fuse.engine \
...
def main():
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            # generate inverse_c12l0 and inverse_feat2bev
            n = 1 # batch size
            inverse_c12l0 = []
            for sensor2keyego in sensor2keyegos:
                c12l0 = sensor2keyego[:, 0:1, :, :]
                bda_ = torch.zeros((n, 1, 4, 4), dtype=bda.dtype).to(bda)
                bda_[:, :, :3, :3] = bda.unsqueeze(1)
                bda_[:, :, 3, 3] = 1
                c12l0 = bda_.matmul(c12l0)
                inverse_c12l0.append(torch.inverse(c12l0))
            inverse_c12l0 = torch.cat(inverse_c12l0, dim=0).contiguous().unsqueeze(0)
            
            feat2bev = torch.zeros((3, 3), dtype=bda.dtype).to(bda)
            feat2bev[0, 0] = model.img_view_transformer.grid_interval[0]
            feat2bev[1, 1] = model.img_view_transformer.grid_interval[1]
            feat2bev[0, 2] = model.img_view_transformer.grid_lower_bound[0]
            feat2bev[1, 2] = model.img_view_transformer.grid_lower_bound[1]
            feat2bev[2, 2] = 1
            feat2bev = feat2bev.view(1, 3, 3)
            inverse_feat2bev = torch.inverse(feat2bev)
            
            sensor2keyegos = torch.cat(sensor2keyegos, dim=0).contiguous().unsqueeze(0)
            ...
            if occ_filename[0][0] == occ_filename[0][1] and occ_filename[0][0] == occ_filename[0][2] and \
                    ...
                    occ_filename[0][0] == occ_filename[0][7] and occ_filename[0][0] == occ_filename[0][8]:
                bev_feat, _, feat_2d = model.prepare_bev_feat_trt(img, ranks_depth, ranks_feat, ranks_bev, \
                    interval_starts, interval_lengths, mlp_input)
                for idx in range(1, 9): # prev frame num
                    feat_prev.append(bev_feat)   
            else:
                try:
                    assert occ_filename[0][1] in model.occ_memory
                    assert occ_filename[0][2] in model.occ_memory
                    assert occ_filename[0][3] in model.occ_memory
                    assert occ_filename[0][4] in model.occ_memory
                    assert occ_filename[0][5] in model.occ_memory
                    assert occ_filename[0][6] in model.occ_memory
                    assert occ_filename[0][7] in model.occ_memory
                    assert occ_filename[0][8] in model.occ_memory
                except:
                    print('There is something wrong with the dataloader')
                    print('Activate backup strategy')
                    bev_feat, _, feat_2d = model.prepare_bev_feat_trt(img, ranks_depth, ranks_feat, ranks_bev, \
                        interval_starts, interval_lengths, mlp_input)                    
                    model.occ_memory[occ_filename[0][0]] = bev_feat
                for idx in range(1, 9):
                    feat_prev.append(model.occ_memory[occ_filename[0][idx]])
            feat_prev = torch.cat(feat_prev, dim=0).contiguous().unsqueeze(0) # add batch size
            ...
            #                         sensor2keyegos=sensor2keyegos.cuda().contiguous(),
            #                         bda=bda.cuda().contiguous(),
            #                         inverse_c12l0=inverse_c12l0.cuda().contiguous(),
            #                         inverse_feat2bev=inverse_feat2bev.cuda().contiguous(),
            ...
            model.occ_memory[occ_filename[0][0]] = feat_2d_ret

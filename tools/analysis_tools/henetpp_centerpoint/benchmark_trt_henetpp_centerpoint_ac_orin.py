...
'''
henetpp_bev:
PYTHONPATH='.' python ./tools/analysis_tools/henetpp_centerpoint/benchmark_trt_henetpp_centerpoint_ac_orin.py \
configs/henetpp/changan_rc_multitask_small_res_deploy_centerpoint.py \
work_dirs/changan_finetune/epoch_24.pth \
mmdeploy/changan_centerpoint/changan_fp16_fuse.engine \
--samples 1600 --postprocessing --eval

PYTHONPATH='.' python ./tools/analysis_tools/henetpp_centerpoint/benchmark_trt_henetpp_centerpoint_ac_orin.py \
configs/henetpp/changan_rc_multitask_small_res_deploy_centerpoint.py \
work_dirs/changan_finetune/epoch_24.pth \
mmdeploy/changan_centerpoint/changan_static_int8_fuse.engine \
--samples 1600 --postprocessing --eval
'''
def parse_args():
...
def main():
    ...
    test_dataloader_default_args = dict(
        samples_per_gpu=1, workers_per_gpu=8, dist=False, shuffle=False)
    test_loader_cfg = {
    ...
    with torch.no_grad():   
        for i, data in enumerate(data_loader):
            ...
            feat_prev = model.get_bev_feat_sequential(img=inputs, img_metas=img_metas)

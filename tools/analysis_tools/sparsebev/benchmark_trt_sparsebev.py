...
'''
PYTHONPATH='.' python ./tools/analysis_tools/sparsebev/benchmark_trt_sparsebev.py \
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if filename[0][0] == filename[0][6] and filename[0][0] == filename[0][12] and \
                ...
                img_feats_curr = model.extract_feat(img_tensor, img_metas)
            ...
            data =  dict(img=img, 
                        lidar2img=lidar2img, 
                        img_timestamp=img_timestamp, 
                        len_img_filenames=len(filename[0]),
                        feat_prev_1=feat_prev_1, 
                        feat_prev_2=feat_prev_2,
                        feat_prev_3=feat_prev_3,
                        feat_prev_4=feat_prev_4)
            ...
            with torch.autograd.profiler.profile(enabled=True, use_cuda=True, record_shapes=False,
                ...
                result = model(**data)
            print(prof.table())
            prof.export_chrome_trace('./GPU_pytorch_stat_oneframe/sparsebev.json')
            exit(0)

...
class ChangAn(object):
    ...
    def save_data(self, path):
        ...
        with open(osp.join(path,'train_infos_full.pkl'), 'wb') as fid:
            pickle.dump(dict(infos=self.data_info_train, metadata=metadata), fid)
        
        with open(osp.join(path,'val_infos_full.pkl'), 'wb') as fid:
    ...
    def _get_info(self, all_dirs):
        ...
        for id in mmcv.track_iter_progress(range(len(all_dirs))):
            ...
            if os.path.isfile(osp.join(dirs, 'info_file/GT_raw/odometry.txt')):
                # print(f'\nbad dirs: {dirs}')
            ...
            for file_name in file_names:
                ...
                if odometry is not None:
                    ...
                    ego_json = ann_info['ego']
                    if 'pose_x' not in ego_json:
                        print(osp.join(dirs, f'merge_json/{file_name}.json'))
                    ego_x = ego_json['pose_x']
                    ego_y = ego_json['pose_y']
                    ego_z = ego_json['pose_z']

                    ego_rot_q = Rotation.from_euler("zyx", [ego_json['yaw'], ego_json['pitch'], ego_json['roll']], False).as_quat()
                    qx, qy, qz, qw = ego_rot_q
                ...
                if len(ann_info['annotations']) == 0:
                    ...
                    for ann in ann_info['annotations']:
                        ...
                        if_valid = True
                        if ann['attributes']['ignore'] != "No":
                            if_valid = False
                        
                        # if ann['attributes']['visible_in_2d'][0] in ['no', 'cam_front_30fov']:
                        #     if_valid = False
                        # if 'no' in ann['attributes']['visible_in_2d']:
                        #     if_valid = False
                        
                        if int(ann['attributes']['occlusion']) == 2:
                            if_valid = False

                        valid_flag.append(if_valid)

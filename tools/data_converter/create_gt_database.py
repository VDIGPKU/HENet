...
def create_groundtruth_database(dataset_class_name,
    ...
    elif dataset_class_name == 'NuScenesDataset_rc':
        dataset_cfg.update(
            use_valid_flag=True,
            pipeline=[
                dict(
    ...
    elif dataset_class_name == 'NuScenesDataset_R':
        dataset_cfg.update(
            use_valid_flag=True,
            pipeline=[
                dict(
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        if dataset_class_name == 'NuScenesDataset_R':
            # gt_boxes_3d, gt_labels = example['ann_infos']
            # gt_boxes_3d, gt_labels = np.array(gt_boxes_3d), np.array(gt_labels)
            gt_boxes_3d = example['gt_bboxes_3d'].tensor.numpy()
            gt_labels = example['gt_labels_3d'].numpy()
            names_map = [
                'car', 'truck', 'construction_vehicle', 'bus', 'trailer', 'barrier',
                'motorcycle', 'bicycle', 'pedestrian', 'traffic_cone'
            ]
            names = [names_map[i] for i in gt_labels]
            annos = None
        else:
        ...
        group_dict = dict()
        if annos is not None and 'group_ids' in annos:
            group_ids = annos['group_ids']
        else:
            group_ids = np.arange(gt_boxes_3d.shape[0], dtype=np.int64)
        difficulty = np.zeros(gt_boxes_3d.shape[0], dtype=np.int32)
        if annos is not None and 'difficulty' in annos:
        ...
        for i in range(num_obj):
            ...
            if (used_classes is None) or names[i] in used_classes:
                ...
                if annos is not None and 'score' in annos:
...
class GTDatabaseCreater:
    ...
    def create(self):
        ...
        elif self.dataset_class_name == 'NuScenesDataset_rc':
            dataset_cfg.update(
                use_valid_flag=True,
                pipeline=[
                    dict(

...
from mmdet3d.datasets import NuScenesDataset_rc
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if not test:
            ...
            for i in range(len(names)):
                if names[i] in NuScenesDataset_rc.NameMapping:
                    names[i] = NuScenesDataset_rc.NameMapping[names[i]]
...
def generate_record(ann_rec: dict, x1: float, y1: float, x2: float, y2: float,
    ...
    if repro_rec['category_name'] not in NuScenesDataset_rc.NameMapping:
        return None
    cat_name = NuScenesDataset_rc.NameMapping[repro_rec['category_name']]

...
def get_image_index_str(img_idx, use_prefix_id=False):
    if use_prefix_id:
        return '{:05d}'.format(img_idx) #因为VOD修改
    else:
        return '{:05d}'.format(img_idx) #因为VOD修改
...
def get_image_path(idx,
                   ...
                   info_type='image_2',
                   use_prefix_id=False):
    return get_kitti_info_path(idx, prefix, info_type, '.jpg', training, #因为VOD数据集是jpg格式 from .png to .jpg
...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        if calib:
            ...
            Tr_imu_to_velo = np.ones((3, 4))
            # np.array([
            #     float(info) for info in lines[6].split(' ')[1:13]
            # ]).reshape([3, 4]) #VOD 没有这个参数
            if extend_matrix:

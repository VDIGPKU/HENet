...
def show_det_data(input, out_dir, show=True):
    """Visualize 3D point cloud and 3D bboxes."""
    img_metas = input['img_metas']._data
    points = input['points']._data.numpy()
    points_radar = input['radar'].tensor.numpy()#._data.numpy()   #change
    '''
    data1 = open(osp.splitext(osp.basename(img_metas['pts_filename']))[0],'w',encoding="utf-8")
    print(points,file=data1)
    data2 = open(osp.splitext(osp.basename(img_metas['pts_filename']))[0]+'radar','w',encoding="utf-8")
    print(points_radar,file=data2)
    '''
    gt_bboxes = None #input['gt_bboxes_3d']._data.tensor
    '''
    if img_metas['box_mode_3d'] != Box3DMode.DEPTH:
        points, gt_bboxes = to_depth_mode(points, gt_bboxes)
    if img_metas['box_mode_3d'] != Box3DMode.DEPTH:
        points_radar, gt_bboxes = to_depth_mode(points_radar, gt_bboxes)
    '''
    filename = osp.splitext(osp.basename(img_metas['pts_filename']))[0]
    filename_radar = filename+'radar'
    show_result(
        points,
        gt_bboxes,#.clone(),
        None,
...
def show_proj_bbox_img(input, out_dir, show=False, is_nus_mono=False):
    ...
    gt_bboxes=None# = input['gt_bboxes_3d']._data
    img_metas = input['img_metas']._data
    ...
    for input in dataset:
        ...
        if vis_task in ['multi_modality-det', 'mono-det']:
            ...
            show_proj_bbox_img(
                ...
                args.output_dir,
                show=args.online,
                is_nus_mono=(dataset_type == 'NuScenesDataset_R')) # change
